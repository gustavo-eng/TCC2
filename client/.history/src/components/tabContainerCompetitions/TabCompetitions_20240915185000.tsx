...
export default function TabCompetitions() {
  ...
  const {user} = selector;
  console.log('user ffrom TabCompetition')
  const user2: any = getSession("user");
  ...
  const handleCardAction = (id: any) => {
    if (user2.role === "athlet") {
      setIdEvent(id?.toString());
      setModalRegistration(true);
    } else if (user2.role === "gym") {
        ...
        {filteredEvents?.map((el, idx) => (
          <Fragment key={idx}>
            <CardAction
              ...
              titleButton={`${user2.role == "athlet" ? "INSCREVER-SE" : "VER INSCRIÇÕES"}`}
            />
          </Fragment>
        ))}
      </div>
