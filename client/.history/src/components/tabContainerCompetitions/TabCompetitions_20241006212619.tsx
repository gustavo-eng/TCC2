...
function findEventById(data: any,idEvent: number) {
  const event = data.find((item: any) => item.idEvent === idEvent);
...
export default function TabCompetitions() {
  ...
  const getAllRegistrions = async () => {
    try{
      const response: any = await  client.payments.listAll();
      if(response.status) {
        ...
        {filteredEvents?.map((el, idx) => (
          <Fragment key={idx}>
            <CardAction
              ...
              btn={findEventById(allRegistration, Number(el?.idEvent)) &&  user.role == "athlet" ? true : false}
