    ...
    useEffect(() => {
      if (events) {
        const filtered = events.filter((event) =>
          event.name.toLowerCase().includes(searchText.toLowerCase()) || // Verifica se o nome contém o texto de busca
          formatDate(String(event.startDate)).includes(searchText) // Verifica se a data contém o texto de busca
        );
        setFilteredEvents(filtered); // Atualiza a lista de eventos filtrados
      }
    }, [searchText, events]);
