...
function countEventById(data: any,idEvent: number) {
  try {
    const eventLength = data.filter((item: any) => item?.Event?.idEvent === idEvent).length;
      ...
      <div className="w-fit grid grid-cols-2 lg:grid-cols-4 gap-0">
        {filteredEvents?.map((el, idx) => (
          <Fragment key={idx}>
            ...
            >
              <Button
              className="w-full h-7   rounded-md bg-red-500/80 hover:bg-red-600/70 text-white font-normal text-xs lg:text-sm mb-1 mt-2"
              label={'Deletar'}
              classNameLabel="text-xs lg:text-md"
              onClick={() => deleteEvent(String(idEvent))}
            />
            </CardAction>
          </Fragment>
        ))}
