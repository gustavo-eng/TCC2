...
export default function BellNotifications({
  ...
  const acceptAthlet = (id: string | number) => {
    console.log(Number(id));
    let response = client.gym.acceptSolicitation(id);
  ...
  const getSolicitation = async () => {
    ...
    console.log('response getSolicitation ', response)
      ...
      >
        ...
        <div
          className={`divide-y divide-gray-600   ${
            solicitations && Array.isArray(solicitations.length) && solicitations.length > 5 ? "h-96 overflow-y-auto" : ""
          }`}
        >
          { Array.isArray(solicitations) && solicitations?.map((el: any) => {
            return (
                  ...
                  <div className="w-full pl-3">
                    <div className="text-gray-500 text-sm mb-1.5 dark:text-gray-400">
                      <div className="flex flex-row justify-start items-start">
                        <p className="text-white mr-1 lg:w-52">{String(el?.Athlet?.name || "")}</p>
                        <span>Data de nascimento {String(el?.Athlet?.birth || "xx/xx/xxxx")} </span>
                      </div>
                    ...
                    <div className="flex flex-row justify-end">
                      <Button className="bg-green-500/90 hover:bg-green-600 rounded-md text-sm  p-1.5 mr-3" label="Aceitar" onClick={() => acceptAthlet(el?.idRequest)}/>
