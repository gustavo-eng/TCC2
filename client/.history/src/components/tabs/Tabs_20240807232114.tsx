    ...
    return (
        <div className="w-full">
            <TabGroup selectedIndex={selectedTabIndex}>
                <TabList className="flex  lg:flex-row justify-center">
                    {tabs.map((tab) => (
                        <Tab
                            key={tab.id}
                            onClick={() => {
                            ...
                            className={({ selected }) =>
                                twMerge(
                                    'min-w-[9rem] min-h-[2.8rem] font-bold mr-3 mb-1 hover:text-green-700 hover:bg-gray-200 rounded-md  focus:outline-0 flex gap-2 justify-center items-center transition duration-100',
                                    selected
