...
const Input = forwardRef<HTMLInputElement, InputProps>(function ({
    ...
    return (
        <div className={twMerge('flex  text-md items-start w-full', className)}>
            <div className="relative w-full rounded  flex  flex-col items-start justify-start">
                {label && (
                    <label className="text-gray-500 text-sm font-semibold">{label} {!isOptional && <span className="text-red-700"> *</span>} </label>
                )}
                {icon && (
                    <div className="pointer-events-none absolute inset-y-0 flex flex-row justify-end items-center pl-3 z-10">
