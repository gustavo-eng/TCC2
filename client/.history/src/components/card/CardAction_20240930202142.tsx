...
export default function CardAction({
   ...
   const detectImg = () => {
        if(name?.trim().toLowerCase().includes('bra') || name?.trim().toLowerCase().includes('nac')) {
            setSrcImg("src/assets/cbj.webp")
        } else {
            setSrcImg("src/assets/olimpicGame.webp")
        }
   ...
   useEffect(() => {
    ...
    return (
        <>
            <div className="bg-white flex flex-col justify-between  lg:w-[16vw] rounded-md m-1 shadow pb-10 ">
                <div className="hidden" id={idEvent} >{idEvent}</div>
                <div className="hidden" >{type}</div>
                <div className="lg:p-1 p-0 m-0 bg-red-400"> {srcImg}
                    <img
                        src={srcImg}
                        alt="img"
                        //style={{ width: "00px", height: "200px", border: "2px solid red" }}
