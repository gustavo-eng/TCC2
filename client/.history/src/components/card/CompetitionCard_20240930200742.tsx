...
const CompetitionCard: React.FC<CompetitionCardProps> = ({
  ...
  //console.error(`http://localhost:3001/${idAthlete}/${String(voucherPath).split('\\').pop()}`)
  ...
  useEffect(() => {
    if (idAthlete && voucherPath) {
      ...
      setImageUrl(null); // ou algum valor padrão
    }
    ...
    <div className="bg-white shadow flex flex-col justify-start  lg:w-[15vw] rounded-md m-1 h-fit p-1">
      ...
      <div className="mt-2 h-36">
        <img
          ...
          src={imageUrl ? imageUrl : "src/assets/olimpicGame.webp"}
          alt="img"
          className="h-full w-full rounded"
        />
      </div>
      {status == 'Rejeitado' && description && (
        <div className="p-1  lg:h-fit">ddd
