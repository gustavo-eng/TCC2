...
export default function CardAction({
   ...
   const detectImg = () => {
        ...
        } else if(name?.trim().toLowerCase().includes('mund') || name?.trim().toLowerCase().includes('int') || name?.trim().toLowerCase().includes('cir') || name?.trim().toLowerCase().includes('ijf')) {
    ...
    return (
        <>
            <div className="bg-white flex flex-col justify-between  lg:w-[16vw] h-fit rounded-md m-1 shadow pb-1 ">
                <div className="hidden" id={idEvent} >{}</div>
