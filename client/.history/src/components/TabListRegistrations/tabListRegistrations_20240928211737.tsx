...
function TabListRegistrations(){
    ...
    return (
        <div className="w-screen flex flex-col justify-center items-center p-3">
            {/*title={`${registrations && registrations.length == 1 ?  `Inscrição do evento: ${registrations?.[0]?.Event?.description || ""}` : `Inscricoes`}`} */}
            {JSON.stringify(registrationNew)}
            <GlobalTile title={`${registrationNew && registrationNew.length == 1 ?  `Inscrição do evento: ${registrationNew?.[0]?.Event?.description || ""}` : `Inscricoes e pagamentos`}`}/>
