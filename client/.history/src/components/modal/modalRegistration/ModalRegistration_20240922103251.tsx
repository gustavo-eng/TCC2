...
export default function ModalRegistration({
    ...
    const [classCategory2, setClassCategory2] = useState<any>();
    ...
    async function getOptionsClassCategoryByGender(gender?: string) {
        // Filtra as categorias pelo gender fornecido
        let response = await  client.category.get();
        setAllCategory(response.payload)

        let filteredCategories = allCategory.filter((category: any) => category.gender === gender);

        // Extrai os classCategory únicos dos registros filtrados
        const uniqueClassCategories = Array.from(new Set(filteredCategories.map((category: { classCategory: any; }) => category.classCategory)));

        // Mapeia os classCategories para o formato { value, label }
        let t =  uniqueClassCategories.map(classCategory => ({
          value: classCategory,
          label: classCategory,
        }));
    ...
    */
    ...
    //Category
    getOptionsGender();
   // if(optionsGenderGenerate[0]) getOptionsClassCategoryByGender(optionsGenderGenerate[0].value)
   getOptionsClassCategoryByGender('Feminino')
