...
export default function ModalRegistration({
    ...
    useEffect(() => {
        if (isOpen) {
            getOptionsGender(); // Chama ao abrir o modal
        }
    }, [isOpen]);
     ...
     const getOptionsGender = async () => {
        try {
            ...
            setOptionsGenderGenerate(optionsGender || [{ value: 'null', label: 'Nenhum informado' }]);
        } catch (error) {
            console.error("Erro ao buscar gêneros", error);
            setOptionsGenderGenerate([{ value: 'null', label: 'Nenhum informado' }]); // Fallback
        }
    }
