...
const validateButton = (onClick: () => void, idPayment: any) => {
  return (
...
function TableRegisters({
  ...
  const [selectedAthleteName, setSelectedAthleteName] = useState<string | undefined>(undefined);
  ...
  const openModalViewRegistraion = () => {
  ...
  const [colDefs] = useState<ColDef[]>([
    ...
    },
    {
      headerName: "Acao",
      cellRenderer: (props: any) => {
        console.warn(' ***  props  *** ' , props?.['data']?.['idPayment'])
        return  validateButton(openModalViewRegistraion, props?.['data']?.['idPayment'])
      },
      cellStyle: { textAlign: "center" },
      flex: 0.4,
    },
  ...
  const optionsClass = [
    { value: "Masculino", label: "Senior" },
    { value: "Feminino", label: "Junior" },
    { value: "Feminino", label: "Mirim" },
  ];
