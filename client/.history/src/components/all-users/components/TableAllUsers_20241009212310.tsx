...
function TableAllUsers({
  ...
  useEffect(() => {
    setOptionsCities(structNestedOptions(tableJSON, ['city'], true))
  ...
  const filterCityStartsWith = useCallback((el: string) => {
    gridRef.current!.api.setColumnFilterModel('city', {
      type: "startsWith",
      //filter: el !== 'all' ||  el !== null  || el !== ''  ? `${el}` : null,
      filter: el !== 'all' ? `${el}` : null,
    }).then(() => {
      gridRef.current!.api!.onFilterChanged();
    });
  }, [])
  const filterGymNameStartsWith = useCallback((el: string) => {
    gridRef.current!.api.setColumnFilterModel('Gym.name', {
      type: "startsWith",
      //filter: el !== 'all' ||  el !== null  || el !== ''  ? `${el}` : null,
      filter: el !== 'all' ? `${el}` : null,
    }).then(() => {
      ...
      <div className="flex flex-col lg:flex-row justify-start mt-2">
        <div className="flex lg:none mb-8">
          <Select
            ...
            onChange={(e) => filterGymNameStartsWith(e.target.value)}
          />
          <Select
            id="city"
            name="city"
            ...
            classNameSelect="bg-white rounded-md border-gray-400 hover:border-green-500"
            className="w-full lg:w-[10vw] mt-0 lg:mr-2 bg-gray-50"
            onChange={(e) =>  filterCityStartsWith(e.target.value)}
          />
        </div>
      </div>
