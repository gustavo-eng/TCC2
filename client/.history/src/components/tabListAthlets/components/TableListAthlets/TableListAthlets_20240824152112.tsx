...
function EditButtonsTable(e: any) {
  return (
    <div className="h-full flex flex-row justify-center items-center">
      <Edit
        width={17}
        className="text-green-900 mr-4 cursor-pointer"
        onClick={() => console.log(" Ediiteeee", e)}
      />
    </div>
  );
}

function RemoveButtonTable(e: any) {
  return (
    <div className="h-full flex flex-row justify-center items-center">
      <Trash
      width={17}
      className="text-red-800 cursor-pointer"
      onClick={() => console.log(" Lixoooo ")}
    />
  </div>
  )
}
...
function TableListAthlet({
  ...
  const [colDefs] = useState<ColDef[]>([
    ...
    },
    {
      flex: 0.2,
      headerName: "Editar",
      cellRenderer: EditButtonsTable,
    },
    {
      flex: 0.2,
      headerName: "Remover",
      cellRenderer: RemoveButtonTable,
    },
  ]);
