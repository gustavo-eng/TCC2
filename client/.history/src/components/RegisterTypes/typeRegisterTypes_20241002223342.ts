...
export const typeRegisterScheema = z.object({
    ...
    weight: z.array(z.object({
        weight: z.string({message: 'Must be a string'})
    }))
    //z.string({message: 'Deve ser um caractere '}).min(5, {message: 'Minimo 5 caracteres'}).max(255, {message: 'Máximo 255 caracteres'}),
})
