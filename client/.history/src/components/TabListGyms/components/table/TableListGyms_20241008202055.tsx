...
function TableListGyms({
  ...
  const [idGym, setIdGym] = useState<string | null>(null);

  // Função para buscar academias (gyms)
  const fetchGyms = async () => {
    try {
      const response = await client.gym.getAll(); // Ajuste para a função de busca de gyms
      setRowData(response.data); // Atualiza os dados da tabela
    } catch (error) {
  ...
  const deleteGym = async (idGym?: number) => {
    try {
      ...
      if (response.status) {
        ...
        await fetchGyms(); // Atualiza os dados da tabela após a exclusão
      } else {
  ...
  useEffect(() => {
    fetchGyms(); // Busca os dados das academias ao carregar o componente
  }, [tableJSON]);
