...
export default function ForgotPassword() {
  ...
  const [showPasswordFields, setShowPasswordFields] = useState<boolean>( resetToken ? true :false);
  ...
  const sendEmail = async (email: string) => {
    try {
      const response: responseGenericApi =
        await client.resetPassword.sendEmailReset(String(email));
      if (response.status) {
  ...
  const changePassword = async () => {
    if (!newPassword || newPassword.length < 5) {
      ...
      if (newPassword && newPassword.length < 5)
        setErrorPassword("A senha deve conter no mínimo 5 caracteres. ");
    } else {
      if (resetToken) {
        const response = await client.resetPassword.resetPassord(
          String(resetToken),
          String(newPassword)
        );
        if (response.status) {
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      {!showPasswordFields ? (
        <label className="text-base font-medium mb-6 text-green-800/80 ">
          Informe seu e-mail para obter um link de redefinição de senha. Caso o
          email for válido, o sistema irá liberar os campos para configura a
          nova senha.
        </label>
      ) : (
      ...
      {showPasswordFields && (
        <div className="">
          <PasswordInput
            ...
            onChange={(e: any) =>
              setNewPassword(String(e?.target?.value) || null)
            }
      ...
      <ButtonPrimary
        ...
        onClick={() =>
          !showPasswordFields ? sendEmail(email as string) : changePassword()
        }
      />
    </div>
  );
