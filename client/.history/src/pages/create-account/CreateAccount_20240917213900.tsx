 ...
 // const watchedValues = watch();
  ...
  const setAllGyms = async () => {
    ...
    if (Array.isArray(response?.payload)) {
      const newGyms = response.payload.map((el: { idGym: any; name: any }) => ({
        value: el.idGym,
        label: el.name,
      }));

      setGyms((prevGyms: any[]) => {
        // Filtra novos gyms que ainda não estão no estado atual
        const uniqueGyms = newGyms.filter(
          (newGym: any) => !prevGyms?.some((gym) => gym.value === newGym.value)
        );

        // Retorna a lista anterior mais os novos gyms únicos
