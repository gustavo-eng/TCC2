    ...
    <form onSubmit={handleSubmit(onSubmit)} className=" w-full">
      <div className="flex flex-col  items-center gap-4 pl-4  lg:w-full w-screen ">
        ...
        <div className="w-full flex flex-col items-center gap-2 ">
          ...
          <Input label="CPF" isOptional className="w-full lg:w-1/2" placeholder={`${user?.cpf || ""}`}  {...register('cpf')} errorMessage={errors?.cpf?.message ?? ''}/>
          <Input label="RG" isOptional className="w-full lg:w-1/2" placeholder={`${user?.rg || ""}`} {...register('rg')} />
          <Input label="Telefone" isOptional className="w-full lg:w-1/2" placeholder={`${user?.phone || "(xx)xxxx - xxxx"}`}   />
          <Input label="Nascimento" type="date" isOptional className="w-full lg:w-1/2 cursor-pointer" placeholder={`${user?.birth || ""}`} />
          <h1 className="w-1/2 text-green-500 text-center font-semibold text-xl">Endereço</h1>
          <Input label="Cidade" isOptional className="w-full lg:w-1/2" placeholder={`${user?.city || null}`} />
          <Input label="Bairro" isOptional className="w-full lg:w-1/2" placeholder={`${user?.neighborhood || ""}`}/>
          <Input label="Número" isOptional className="w-full lg:w-1/2" placeholder={`${user?.number || null}`} />
          <Button
            type="submit"
