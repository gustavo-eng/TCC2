...
//import TabTestModal from "../comerrorponents/tabTestModal/TabTestModal";
...
export default function LayoutMain({ componentName = "tab" }) {
  ...
  useEffect(() => {
    console.log(`user in layoutMain`, user)
  ...
  const tabs = useMemo(() => {
    if (user?.role.includes("athlet")) {
    ...
    } else if (user?.role.includes("gym")) {
      return [
        { id: "tabCompetition", icon: <CalendarPlus size={25} /> },
      ...
      return [
        ...
        { id: "registerTypes", icon: <User size={25} /> },
        /*
        {
        ...
        <nav className="  px-4 lg:px-6 pt-1 pb-0 lg:pb-0  bg-gradient-to-br from-slate-900 to-slate-800">
          <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div className="flex items-center lg:order-2">
              {user?.role === "gym" && <BellNotifications />}
              <UserDropDown />
              <button
                onClick={toggleMenu}
