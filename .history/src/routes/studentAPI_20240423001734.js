...
router.post('/', (req, res) => {
    ...
    studentDAO.save(name, email, password, cpf, gymId).then(student => {
        ...
        let month = new Date(dateFromDB.toString()).getMonth() + 1 < 10 ? '0' + (new Date(dateFromDB.toString()).getMonth()).toString() : new Date(dateFromDB.toString()).getMonth();
        let data = `${new Date(dateFromDB.toString()).getDate()}/${month}`
