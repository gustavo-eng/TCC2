...
router.post('/login', (req, res) => {
    ...
    requirementsDAO.verifyAuthenticationStudent(email, password).then(requirement => {
        if (requirement.payload.aproved) {
            console.log("requirement.payload.aproved accepted")
            console.log(requirement.payload.aproved)
            res.status(200).json(success(requirement, "payload", "Usuario devidamente autenticado"));
        } else {
            console.log("requirement.payload.aproved NOT accepted")
            console.log(requirement.payload.aproved)
            res.status(500).json(success(requirement, "payload", "Usuario nao autenticado"));
        }
