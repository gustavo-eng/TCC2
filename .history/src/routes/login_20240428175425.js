    ...
    try {
        ...
        if (email != "") {
            ...
            if (user) {
                ...
                console.log('user')
                console.log(user)
                //todo colocar verificacao da solicitacao para o aluno
                //colocar aqui dentro  requirementsDAO.verifyAuthenticationStudent(email, password).then(requirement => {
                ...
                */
            let token = jwt.sign({ user: email, userPermission: "fprj", userId: 2 }, process.env.SECRET_JWT, { expiresIn: '24h' });
            res.json({ isLogged: true, token: token, msg: 'User successfully authenticated' });
