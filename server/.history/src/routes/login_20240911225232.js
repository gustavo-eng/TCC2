...
router.post("/signIn", async (req, res) => {
    ...
    try {
        ...
        if (!passwordIsValid)  res.status(statusCode.UNAUTHORIZED).json(fail("Invalid Credentials"));
        ...
        if (user.role.includes('athlet')) {
            ...
            if (isAccepted) {
                let token = jwt.sign({ user: user.name, userPermission: user.role, userId: user.idAthlete }, process.env.SECRET_JWT, { expiresIn: '24h' });
                 res.json({user: user ,status: true, isLogged: true, token: token, msg: 'User successfully authenticated', userPermission: user.role });
            } else {

                 res.status(statusCode.NOT_FOUND).json(fail("Request pending..."));
         ...
         res.status(statusCode.UNAUTHORIZED).json(fail("Error server. Error -> " + err));
