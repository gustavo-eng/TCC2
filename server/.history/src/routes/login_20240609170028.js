...
router.post('/signIn', async (req, res) => {
    ...
    try {
        ...
        if (user.role.includes('athlet')) {
            ...
            if (isAccepted) {
                let token = jwt.sign({ user: user.name, userPermission: user.role, userId: user[`cod_${user.role}`] }, process.env.SECRET_JWT, { expiresIn: '24h' });
            } else {
                res.status(401).json(fail("User not authenticated"));
            }
