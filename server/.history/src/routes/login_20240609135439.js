...
router.post('/signIn', (req, res) => {
    ...
    try {
        let user10 = {}
        let user = verifyUser({ email: email, password: password }).then(user => {
            console.log('[[[[[[[[')
            console.log(user)
            user10 = { ...JSON.stringify(user) }
            return user
        });
        console.log('user')
        console.log(user)
        ...
        db.Athlet.findOne({ where: { email: email, password: password } }).then(el => {
            console.log('eeeeeeeeeelll ')
            console.log(el.get({ plain: true }))
