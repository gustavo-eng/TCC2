...
router.post("/aprove/:idPayment", validate(paymentSchema), registration.aprovePayment);

//router.put("/reprove/:idPayment", payment.reprovePayment);

//Rota direcionada para colocar comentarios de pagamentos reprovados
router.put("/reprove/:idPayment", validate(paymentSchema), registration.reprovePayment);


//Delete payment
router.delete("/:idPayment", registration.delete)

router.put("/", (req, res) => res.send(`<h1>Ola mundo - ${req.body.idEthlete} </h1>`))
