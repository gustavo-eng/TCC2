...
exports.findAll = async (req, res) => {
    const sqlQuery = `
        SELECT e.*, t.*
        FROM Events e
        LEFT JOIN TypeEvents t ON e.typeEventId = t.id
    `;

    try {
        const [results, metadata] = await sequelize.query(sqlQuery);
        return res.status(statusCode.OK).json(success(results, "payload", "Event listed successfully"));
    } catch (err) {
        console.error(err);
        return res.status(statusCode.INTERNAL_SERVER_ERROR).json(fail("Events not found"));
    }
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        await Event.destroy({ where: { idEvent: idEvent } }).then(_ => {
            ...
            return res.status(500).json(fail("Fail to delete event. Erro -> ", err));
        });
