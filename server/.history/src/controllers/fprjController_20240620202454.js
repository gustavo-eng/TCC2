...
exports.update = async (req, res) => {
    try {
        ...
        const fprj = await Fprj.findByPk(idFprj); // Use await
        if (!fprj) return res.status(statusCode.NOT_FOUND).json(fail("No Fprj found"));
        ...
        await fprj.save().then(event => {
            ...
            res.status(statusCode.OK).json(success(event, "payload", "Event updated"));
