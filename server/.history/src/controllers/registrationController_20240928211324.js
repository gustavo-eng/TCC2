...
exports.findEspecific = async (req, res) => {
  try {
    const {idEvent} = req.query;
    const registration = await Registration.findAll({
      where: { idEvent },
      include: ["Event", "Category", "Athlet"],
    });

    if(!registration || registration.length === 0) return res.status(statusCode.NOT_FOUND).json(fail("Registration not found."))
    return  res.status(statusCode.OK).json(success(registration, "payload", "Payment returned"))
