...
exports.create = async (req, res) => {
  ...
  try {
    ...
    await Category.create(newCategory).then(data => {
      res.status(statusCode.OK).json(success(data, "payload", "Category saved successfully"));

    }).catch(err => {

      res.status(statusCode.BAD_REQUEST).json(fail("Fail to create category. Erro -> ", err));

    });

  } catch (err) {
    res.status(statusCode.INTERNAL_SERVER_ERROR).fail("Error server. Error --> " + err);
  }
