...
async function cleanObsoletAthlets({ XDays }) {
    ...
    //Deixa para deletar pagamento. Nao farei o cron para o pagamento. Pois vai que a fprj quer vizualizar
    //os pagamentos passados.
    allRequests.forEach(async (request) => {
        ...
        if (diffInDays > XDays) {
            ...
            await Requests.destroy({ where: { idAthlete: request.idAthlete } });
        }
    });
